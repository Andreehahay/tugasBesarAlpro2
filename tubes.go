...
const MAX_PERANGKAT = 50 // kapasitas maksimum array statis (Erlan)
...
// hitungTotalKonsumsi menjumlahkan seluruh konsumsi harian semua perangkat (Andre)
...
func tambahPerangkat() {
	...
	// membentuk struct perangkat baru dan menyimpannya ke array statis (Andre)
...
func ubahPerangkat() {
	...
	// memperbarui ruangan jika input baru tidak kosong (Andre)
...
func menuCariPerangkat() {
	...
	if pilihan == 1 {
		...
		// mengurutkan salinan array dulu sebelum binary search dijalankan (Andre)
...
func menuUrutkanPerangkat() {
	...
	// menanyakan arah pengurutan setelah kategori dipilih (Andre)
	fmt.Println("  Urutan:")
	...
	// menyalin array global ke sementara agar urutan asli tidak berubah (Andre)
	var arrSortir [MAX_PERANGKAT]Perangkat
	salinArray(&arrSortir, jumlahPerangkat)
...
func tampilkanStatistik() {
	...
	// menampilkan maksimal 5 perangkat paling boros di bagian atas (Andre)
...
func main() {
	...
	// program terus berjalan sampai pengguna memilih menu keluar (Andre)
	berjalan := true
	for berjalan {
		cetakMenuUtama()
